...
st.markdown('<div class="metrics-row-container">', unsafe_allow_html=True)
...
if not df_pauta.empty:
    ...
    # 1. COSTE ANUNCIO
    st.markdown(f"""
    <div class="metric-item pauta">
        <div class="metric-icon">💰</div>
        <div class="metric-content">
            <div class="metric-label">COSTE ANUNCIO</div>
            <div class="metric-value pauta">${format_number(coste_anuncio_sum)}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="metric-item pauta">
        ...
        <div class="metric-content">
            ...
            <div class="metric-value pauta">{format_number(visualizaciones_videos_sum)}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="metric-item pauta">
        ...
        <div class="metric-content">
            ...
            <div class="metric-value pauta">{format_number(nuevos_seguidores_sum)}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if not df_followers.empty and 'Seguidores_Totales' in df_followers.columns:
    if not df_followers['Seguidores_Totales'].dropna().empty:
        total_followers_series = df_followers['Seguidores_Totales'].dropna()
        total_seguidores = int(total_followers_series.iloc[-1])
...
st.markdown('<div class="grafica-buttons-row">', unsafe_allow_html=True)

# Crear botones usando columnas de Streamlit para asegurar que estén en la misma línea
